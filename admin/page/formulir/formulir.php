            ...
            <div class="card-body">
                ...
                <table id="dtTabel" class="table table-striped table-sm w-100 nowrap">
                    ...
                    <tbody>
                        <?php 
                        $nomor = 1; 
                        $ambil = $con->query("SELECT * FROM berkas NATURAL JOIN pembayaran ORDER BY tgl_berkas DESC");
                            ...
                            <tr> 
                                ...
                                <td>
                                    ...
                                    $data_barang = mysqli_fetch_array($sql_barang);
                                        ...
                                        if ($data_barang['status_santri'] == 'Berkas Sudah di Upload, Menunggu Verifikasi' ) {
                                            echo "<br><br>";
                                            echo "<form action='?page=formulir&aksi=verifikasi' method='POST'> 

                                                    <label for='status_santri'>Status Berkas:</label>
                                                    <select name='status_santri' id='status_santri' class='form-control' required>
                                                        <option value='Acc - Tunggu jadwal tes diumumkan'>Acc - Tunggu jadwal tes diumumkan</option>
                                                        <option value='Tolong upload ulang berkas anda'>Tolong upload ulang berkas anda</option>
                                                    </select>

                                                    <input type='hidden' name='nisn' value='{$data_barang['nisn']}'>
                                                    <input type='hidden' name='nama_lengkap' value='{$data_barang['nama_lengkap']}'>
                                                    <input type='hidden' name='notelp' value='{$data_barang['notelp']}'>
